...
if ! rpmbuild -ba -D "dist .fc40" -D "_sourcedir $(pwd)" -D "_builddir $(pwd)" -D "_srcrpmdir $(pwd)" -D "_rpmdir $(pwd)"  kdump-utils.spec; then
...
arch=$(uname -m)
rpm_name=$(rpmspec -D "dist .fc40" -q --queryformat '%{NAME}-%{VERSION}-%{RELEASE}.%{ARCH}' kdump-utils.spec)
